...
def add_to_redis_set(redis_client, value):
    redis_client.sadd('room_list', value)
    print(f"Added '{value}' to room_list set in Redis")

def remove_from_redis_set(redis_client, value):
    redis_client.srem('room_list', value)
    print(f"Removed '{value}' from room_list set in Redis")

def list_redis_set(redis_client):
    elements = redis_client.smembers('room_list')
    if elements:
        for element in elements:
            print(element.decode('utf-8'))  # Redis返回的数据是字节序列，我们需要解码成字符串
    else:
        print("No elements found in 'room_list'")
