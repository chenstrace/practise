...
func main() {
    ...
    for _, initialDevice := range initialDevices {
        runtime.SetFinalizer(initialDevice, func(d *DeviceInfo) {
            fmt.Printf("Device %s has been garbage collected\n", d.ID)
        })
    }

    ctx.UpdateDevices("FOS", initialDevices)

    newDevices := map[string]*DeviceInfo{
        "device1": {
            ID:          "device1",
            Description: "Updated description (data2)",
    ...
    go func() {
        ...
        fmt.Println("Writer: Updating devices...")
        // 更新设备数据
        ctx.UpdateDevices("FOS", newDevices)
        fmt.Println("Writer: Devices updated to data2")
